...
i1, i2, v2 = calcular_transformador(Uf, Rc, k, f, R1, L1, C1, True)
...
for i in range(len(K)):
    ...
    plt.title("Módulo das correntes em função das correntes (K variando)", pad=20)
    plt.xlabel("Frequência $f$",labelpad=20)
    plt.ylabel("Correntes [A]",labelpad=20)
    plt.savefig(f'Coef_acoplamento_{K[i]}.png', dpi=300)

# Abaixo estamos pegando todos os plots gerados e criando uma animação em gif
...
# ---------------------------------------------------------------------------- #
#                 Ensaio das Potência em Função da Frequência                  #
# ---------------------------------------------------------------------------- #
...
for f in F:
    i1, i2, v2 = calcular_transformador(Uf, Rc, k, f, R1, L1, C1, False)
    i_rc = v2 / Rc
    # Potências:
...
# ---------------------------------------------------------------------------- #
#           Tensão de Pico No Secundário em Função do Coef. de Acoplamento     #            #
# ---------------------------------------------------------------------------- #
