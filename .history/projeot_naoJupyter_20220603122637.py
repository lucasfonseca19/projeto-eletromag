...
# ------------------------------ Tensão de Saída ----------------------------- #
print('Com resistor na saída de %.2f Ω, temos:' %Rc)
...
# ---------------------------------------------------------------------------- #
#                 Ensaio das Correntes em função da frequência                 #
# ---------------------------------------------------------------------------- #
F = np.linspace(1000,50000,100) # Intervalo de frequências de 10Hz até 100kHz
I1 = []
I2 = []
print("Módulos das correntes em funcão da frequência da fonte sendo o coef. de acoplamento = {}".format(k))
...
# -------------------------------- Variando K -------------------------------- #
# A seguir plotaremos o mesmo gráfico de corrente em função da frequência, entretanto iremos variar
# o coeficiente de acoplamento k para verificar o comportamento da corrente.
# Iremos criar uma animação para ver o comportamento da corrente em função do coeficiente de acoplamento.
K = np.linspace(0.1,0.9,9)
fig = plt.figure()
axis = plt.axes(xlim = (0,50000), ylim = (0,1))
line1 = plt.axis.plot([], [], label = "corrente I1", color = "b")
line2 = plt.axis.plot([], [], label = "corrente I2", color = "r")
def init():
    line1.set_data([], [])
    line2.set_data([], [])
    return line1, line2
def animate(i):
    i1, i2, v2 = CalcularTransformador(Vt, Rc, K[i], f, R1, R2, L1, L2, C1, False)
    line1.set_data(F, I1)
    line2.set_data(F, I2)
    return line1, line2
anim = animation.FuncAnimation(fig, animate, init_func=init, frames=len(K), interval=200, blit=True)
anim.save('animacao.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
